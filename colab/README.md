# Google Colab student demo

This folder contains a Colab-friendly teaching workflow for `EnergyCommunityEvaluation`.
...
- `colab/EnergyCommunityEvaluation_Student_Demo.ipynb` – a notebook that:
  1. installs the project in Colab,
  2. generates the teaching dataset,
  3. runs `preprocessing.py`,
  4. runs a preprocessing sanity check,
  5. runs `run_parametric_evaluation.py` with a one-click command,
  6. opens key outputs for discussion in class.

## Intended audience

The notebook is designed for students who need a runnable end-to-end example without first collecting a full municipal dataset.
...
1. Open `colab/EnergyCommunityEvaluation_Student_Demo.ipynb` in Google Colab.
...
- The generated REC structure JSON is included as a teaching artifact students can adapt later.
